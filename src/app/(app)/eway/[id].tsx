...
export default function EWayDetailsScreen() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery({ queryKey: ["lorryReceipts"], queryFn: mockApi.getEwayBills });

  const deleteMutation = useMutation({
    mutationFn: () => mockApi.deleteLorryReceipt(id as string),
    onSuccess: () => {
      ...
      >
        ...
        <View className="flex-row items-center justify-between px-4 py-4 bg-white border-b border-natural-200">
          <View className="flex-row items-center flex-1">
            <Pressable
              onPress={() => router.back()}
              className="mr-3 p-2 bg-natural-50 rounded-full"
            >
  ...
  if (isError) {
    return (
      ...
      >
        <Text className="text-body-strong text-natural-500">
          Failed to load E-Way bill.
        </Text>
      </SafeAreaView>
    );
  }
  ...
  const handleAction = () => {
    if (lr.status === "PENDING") {
      Alert.alert(
        "Coming Soon",
        "E-Way Bill generation will be integrated with the GST portal shortly.",
      );
    } else {
    ...
    >
      ...
      >
        ...
        <View className="bg-white rounded-2xl p-5 mb-4 border border-natural-200 shadow-sm">
          ...
          <View className="flex-row justify-between mb-4">
            ...
            <View className="flex-1 items-end">
              ...
              <Text className="text-body-strong text-black uppercase">
                {lr.vehicleNo}
              </Text>
            </View>
          </View>
        ...
        <View className="bg-white rounded-2xl p-5 mb-8 border border-natural-200 shadow-sm">
          <View>
            <Text className="text-caption text-natural-500 mb-2">
              Linked Invoice
            </Text>
            <Pressable
